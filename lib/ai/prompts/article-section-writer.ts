...
export function buildArticleSectionWriterPrompt(input: ArticleSectionWriterInput): { system: string; user: string } {
  ...
  const system = [
    ...
    "- Where a link genuinely helps the reader, write it INLINE in the body as markdown — [descriptive anchor text](https://example.com) — and record the same URL in `links`. A URL that appears only in `links` is invisible: nothing renders that array, so the reader never sees it.",
    "- Prefer linking to a source you were given evidence from, using its URL exactly as supplied. Never invent a URL. Not every section needs a link; one or two across a section is plenty, and none is better than a fabricated one.",
    "- Never make the call to action a link. A source URL behind \"Book a demo\" sends the reader to somebody else's report, which is the opposite of what a call to action is for. Write it as plain text and leave the destination to whoever publishes this.",
